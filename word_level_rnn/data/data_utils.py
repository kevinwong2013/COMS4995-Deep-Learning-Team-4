...
class SequenceWrapper(object):
    ...
    def __init__(self, multivalent_tokens=False, context=None):
        self._seq = tf.train.SequenceExample(context=context)
...
def build_labeled_sequence(seq, class_label, sentence, label_gain=False):
  ...
  Returns:
    ...
    final timestep.
  """
    sequence_example_context = tf.train.Features(feature={
        'sentence': tf.train.Feature(bytes_list=tf.train.BytesList(
            value=[sentence.encode('utf-8')]))
    })
    label_seq = SequenceWrapper(multivalent_tokens=seq.multivalent_tokens, context=sequence_example_context)
