...
import word_level_rnn.adversarial_losses as adv_lib
import word_level_rnn.inputs as inputs_lib
import word_level_rnn.layers as layers_lib
...
flags.DEFINE_integer('rnn_cell_size', 1024,
...
flags.DEFINE_float('learning_rate_decay_factor', 0.9999,
    ...
    * Caches tensors: `lm_embedded`
    ...
    """
        ...
        if FLAGS.single_label:
            ...
            labels = inputs.labels
            weights = inputs.weights
        prediction = layers_lib.predictions(logits)
        eval_ops = {
            'accuracy':
    ...
    def prediction_graph(self, dataset='test'):
        """Constructs prediction evaluation graph.
        Args:
          dataset: the labeled dataset to evaluate, {'train', 'test', 'valid'}.
        Returns:
          prediction
        ...
        eval_ops = {'prediction': logits, 'label': labels, 'index': indices,
                    'input_sentence': inputs.batch.context['sentence']}
