...
import word_level_rnn.train_utils

flags = tf.app.flags
FLAGS = flags.FLAGS

flags.DEFINE_string('pretrained_model_dir', 'tmp/text_train ',
...
def main(_):
  ...
  with tf.device(tf.train.replica_device_setter(FLAGS.ps_tasks)):
    model = word_level_rnn.graphs.get_model()
    train_op, loss, global_step = model.classifier_training()
    word_level_rnn.train_utils.run_training(
        train_op,
        loss,
        global_step,
